...
# ---- FastAPI app ----
app = FastAPI()
app.state.build_id = datetime.now().strftime("%Y%m%d%H%M%S")
...
# Resolve important paths absolutely (prevents “old” templates/static)
BASE_DIR  = Path(__file__).resolve().parent
OUTPUT_DIR = str(Path(OUTPUT_FOLDER) if os.path.isabs(OUTPUT_FOLDER) else (BASE_DIR / OUTPUT_FOLDER))
...
# Templates pinned to ./templates next to this file (no cache in dev)
templates = Jinja2Templates(directory=str(BASE_DIR / "templates"))
templates.env.auto_reload = True
templates.env.cache = {}
templates.env.globals["now"] = lambda: datetime.now()

# Static mounts use absolute paths (works regardless of working directory)
app.mount("/static",    StaticFiles(directory=str(BASE_DIR / "static")),    name="static")
app.mount("/assets",    StaticFiles(directory=str(BASE_DIR / "assets")),    name="assets")
app.mount("/generated", StaticFiles(directory=OUTPUT_DIR),                  name="generated")

@app.get("/_debug/form-path")
async def _debug_form_path():
    src, filename, _ = templates.env.loader.get_source(templates.env, "form.html")
...
def _generate_and_save_pdf(pdf_payload: Dict[str, Any], brand: Dict[str, Any]) -> str:
    ...
    try:
        # Signature A: (payload, out_path=, brand_config=)
        generate_pdf(pdf_payload, out_path=out_path, brand_config=brand)
    except TypeError:
...
def fallback_content(
    ...
    return {
        ...
        "captions": {"Instagram": captions.get("instagram",""), "LinkedIn": captions.get("linkedin",""),
                     "TikTok": captions.get("tiktok",""), "Twitter": captions.get("twitter",""),
                     "Facebook": captions.get("facebook","")},
        "hashtags": hashtags
    }
