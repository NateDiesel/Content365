...
def generate_pdf(payload: Dict[str, Any], output_path: str, brand_config: Optional[Dict[str, Any]] = None) -> str:
    ...
    - Tries premium engine; on failure, writes a minimal fallback PDF.
    ...
    if not (callable(adapt) and callable(export) and callable(fallback)):
        raise RuntimeError(
            "Internal wiring error: missing helpers — "
            f"adapt={bool(callable(adapt))}, export={bool(callable(export))}, fallback={bool(callable(fallback))}"
        )
