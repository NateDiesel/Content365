...
def generate_pdf(content_dict, output_dir="generated_pdfs"):
    ...
    def add_section(title, text, emoji="âœ¨"):
    ...
    # Core Sections
    add_section("Blog Post", content_dict.get("blog_post", ""), "ðŸ“")
    add_section("Lead Magnet", content_dict.get("lead_magnet", ""), "ðŸ“¦")
    add_section("SEO Keywords", ", ".join(content_dict.get("seo_keywords", [])), "ðŸ”")
    ...
    for platform, tags in hashtags.items():
        if tags:
            add_section(f"{platform} Hashtags", " ".join(tags), "ðŸ·ï¸")

    # Premium Tips if included
    if "premium_tips" in content_dict:
        add_section("Pro Tips", "\n".join(content_dict["premium_tips"]), "ðŸ’¡")

    # Footer
    flowables.append(Spacer(1, 0.2 * inch))
    flowables.append(HRFlowable(width="100%", thickness=0.5, color="#dddddd"))
    flowables.append(Spacer(1, 0.1 * inch))
    flowables.append(Paragraph("Generated by Content365.xyz â€“ Â© 2025", footer_style))
